...
use crate::{
    constants::{
        ...
        CONFIG_LINE_SIZE, CUPCAKE_ID, EXPIRE_OFFSET, GUMDROP_ID, PREFIX, W_WL,
    },
    utils::*,
...
pub fn handle_mint_nft<'info>(
    ...
    if !cmp_pubkeys(&current_ix.program_id, &crate::id())
        ...
        && !cmp_pubkeys(&current_ix.program_id, &W_WL)
    ...
    match next_ix {
        Ok(ix) => {
            ...
            if !cmp_pubkeys(&ix.program_id, &W_WL) {
